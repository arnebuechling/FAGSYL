...
if(computer == "Charlie")
{
  ...
  dat_dir <- "/Users/arnebuechling/Library/Mobile Documents/com~apple~CloudDocs/CULS/Projects/Carpathian_tree_growth/Azure_model_files/FAGSYL/"
  out_dir <- "/Users/arnebuechling/Library/Mobile Documents/com~apple~CloudDocs/CULS/Projects/Carpathian_tree_growth/Azure_model_files/FAGSYL/"
  code_dir <- "/Users/arnebuechling/Library/Mobile Documents/com~apple~CloudDocs/CULS/Projects/Carpathian_tree_growth/Azure_model_files/FAGSYL/"
}
...
if (index == 4) { load(paste(spp_list[index],"Model 5BN Results.Rdata")) } else
                { load(paste(spp_list[index],"Model 5BN Full Results.Rdata")) }

if (index == 1) { nspp <- 7 } else
                { nspp <- 5 } 

#setwd("c:/users/canhamc/documents")  # to free up connection to C6
...
if (index == 4) { par <- model_5B_results$best_pars } else
                { par <- model_5BN_full_results$best_pars  }
...
par_lo <- list(PG = rep(0,num_site_plot),
		...
		temp.b = 0.5, temp.lag.b = 0.5,     # the climate "b" parameters are the variances of the gaussian - small to large
...
model_5BN_full_azure_results <- anneal(model=model_5BN, par=par, var=var, source_data=working, par_lo=par_lo, par_hi=par_hi,
			  pdf=linear_dnorm_with_intercept, dep_var="incr_mm", max_iter=iterations)

setwd(out_dir)

write_results(model_5BN_full_azure_results, file = paste(spp_list[index],"Model 5BN Full Azure Results.txt", sep=" "), data=F, print_whole_hist=F)
save(model_5BN_full_azure_results, file=paste(spp_list[index],"Model 5BN Full Azure Results.Rdata",sep=" "))
