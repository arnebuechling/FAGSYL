...
load(paste(spp_list[index],"Model 3 Full Results.Rdata"))
...
par <- model_3_full_results$best_pars
...
model_3_full_annual_prec_results <- anneal(model=pg_size_age_nci_climate_model, par=par, var=var, source_data=working, par_lo=par_lo, par_hi=par_hi,
			  pdf=linear_dnorm_with_intercept, dep_var="incr_mm", max_iter=iterations)

setwd(out_dir)

write_results(model_3_full_annual_prec_results, file = paste(spp_list[index],"Model 3 Full Annual Prec Results.txt", sep=" "), data=F, print_whole_hist=F)
save(model_3_full_annual_prec_results, file=paste(spp_list[index],"Model 3 Full Annual Prec Results.Rdata",sep=" "))
